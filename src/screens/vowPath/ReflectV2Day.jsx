...
// CHANGE: removed STEP.RESULT — closing screen now includes the save confirmation
const STEP = {
  ARRIVAL: 'arrival',
  AUDIO: 'audio',
  INTRO: 'intro',
...
export default function ReflectV2Day() {
  ...
  const handleSaveInteraction = async (data) => {
    ...
    try {
      ...
      setSaving(false)
      advance() // Move directly to closing — no separate "Saved" screen
    } catch (err) {
      console.error(err)
  ...
  // ---- INTRO (REDESIGNED) ----
  // CHANGES: bigger day label, prominent day title at top of reading,
  //          better spacing, more breathing room around the prose
  if (step === STEP.INTRO) {
    return (
      <div style={styles.frame}>
        <div style={styles.phone}>
  ...
  // ---- CLOSING (REDESIGNED — now combines save confirmation + closing line) ----
