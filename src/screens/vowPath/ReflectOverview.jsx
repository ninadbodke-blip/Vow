...
const PHASES = REFLECT_V2_PHASES
...
export default function ReflectOverview() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Query vow_artifacts (v2 system) for completed days
      ...
      <div style={styles.phone}>
        ...
        <div style={styles.header}>
          ...
          <p style={styles.tallyLine}>
            You have gathered <span style={styles.tallyEmph}>{totalCompleted}</span> of {REFLECT_TOTAL_DAYS} days.
          </p>
          <div style={styles.tallyMarks}>
            {Array.from({ length: REFLECT_TOTAL_DAYS }).map((_, i) => {
              const n = i + 1
          ...
          <p style={styles.tallySubtitle}>
            {totalCompleted === 0
              ? 'Begin with Day 1.'
              : totalCompleted === REFLECT_TOTAL_DAYS
                ? 'Reflect is complete.'
