...
import EnvironmentMapper from './mechanics/EnvironmentMapper'
import AnchorPersonPicker from './mechanics/AnchorPersonPicker'
import ReplacementEngineBuilder from './mechanics/ReplacementEngineBuilder'
import IfThenLibraryBuilder from './mechanics/IfThenLibraryBuilder'
import DailyAnchorPicker from './mechanics/DailyAnchorPicker'
import ConversationPlanner from './mechanics/ConversationPlanner'
...
const MECHANIC_COMPONENTS = {
  ...
  anchorPersonPicker: AnchorPersonPicker,
  replacementEngineBuilder: ReplacementEngineBuilder,
  ifThenLibraryBuilder: IfThenLibraryBuilder,
  dailyAnchorPicker: DailyAnchorPicker,
  conversationPlanner: ConversationPlanner,
  vowDrafter: VowDrafter,
  witnessesAndCadence: WitnessesAndCadence,
  finalTap: FinalTap,
}

export default function CommitDay() {
    ...
    try {
      ...
      if (wasLatestDay && !isExploringPastStage(progress, 'commit')) {
        const updates = {
          ...
          current_day: Math.min(dayNumber + 1, COMMIT_TOTAL_DAYS),
          updated_at: new Date().toISOString(),
        }
  ...
  // Determine intro paragraphs (family-aware for Day 4)
  const introParagraphs = (() => {
    if (dayContent?.introByFamily && substance?.family) {
      return dayContent.introByFamily[substance.family] || dayContent.introByFamily.substance
    }
    return dayContent?.intro || []
  })()
      ...
      <div style={styles.frame}>
        <div style={styles.phone}>
          ...
          <div style={styles.arrivalContent}>
            <div style={{ ...styles.dayLabel, letterSpacing: '0.3em', marginBottom: '1.75rem' }}>Day {dayNumber}</div>
