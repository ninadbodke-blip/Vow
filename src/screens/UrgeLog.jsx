    ...
    <div style={styles.frame}>
      <div style={styles.phone}>
        <div style={styles.header}>
          ...
          <p style={styles.headerTitle}>Urge log</p>
        ...
        {loading ? (
          ...
          <div style={styles.emptyState}>
            ...
            <p style={styles.emptyTitle}>No urges logged yet.</p>
            <p style={styles.emptyBody}>
              When the wave hits, we'll guide you through it.
            </p>
          </div>
        ) : (
          <div style={styles.list}>
            <p style={styles.summary}>{urges.length} urges logged</p>
            {urges.map(urge => (
              <div key={urge.id} style={styles.card}>
                <div style={styles.cardHead}>
                  ...
                  {!tracker && (
                    <span style={styles.cardName}>
                      {urge.trackers?.addiction_types?.icon} {urge.trackers?.addiction_types?.name}
                    </span>
                  )}
                  <div style={{flex: 1}}></div>
                  <div style={{textAlign: 'right'}}>
                    <p style={styles.cardDate}>{formatDate(urge.created_at)}</p>
                    <p style={styles.cardRelative}>{formatRelative(urge.created_at)}</p>
                  </div>
                </div>
                {urge.triggers?.length > 0 && (
                  <p style={styles.cardMeta}>
                    Triggers: {urge.triggers.join(' · ')}
                  </p>
                )}
                {urge.notes && (
                  <p style={styles.cardNote}>"{urge.notes}"</p>
                )}
              </div>
            ))}
          </div>
...
const styles = {
  ...
  cardName: { fontSize: '12px', fontWeight: 500, color: '#2A1F15' },
  cardMeta: { fontSize: '11px', color: '#8A7B6A', margin: '8px 0 0' },
