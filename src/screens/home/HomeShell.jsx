...
export default function HomeShell({ progress }) {
  ...
  const [rustleSignal, setRustleSignal] = useState(0)
  const [reflection, setReflection] = useState(null)   // { mood, line } shown floating after a check-in
  ...
  const handleCheckinSaved = (row, reflection) => {
    const isNewToday = !todayCheckin
    setTodayCheckin(row)
    if (isNewToday) setCheckinCount(c => c + 1)   // the tree grows
    // 1) rustle the leaves on the now-visible tree (check-in sheet has closed)
    setRustleSignal(s => s + 1)
    // 2) then, once the boom has played, bring up the quote modal
  ...
  return (
    <div style={styles.frame}>
      ...
      <div style={styles.phone}>
        ...
        {/* The reflection shown after tending the tree (modal) */}
        {reflection && (
